...
const store = {
  ...
  addItemQuantity(itemName, price, quantity) {
    if (!this.isItemInStore(itemName)) {
      inventory[inventory.length] = { name: itemName, price, quantity };
      return quantity;
    }
    let myQuantity;
    for (let i = 0; i < inventory.length; i += 1) {
      if (inventory[i].name === itemName) {
        ...
        myQuantity = inventory[i].quantity;
        break;
      }
    }
    return myQuantity;
  ...
  removeItemQuantity(itemName, quantity) {
    ...
    const myItem = inventory.find((item) => item.name === itemName);
    if (quantity > myItem.quantity) {
      return -1;
    }
